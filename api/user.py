...
def user_register():
    ...
    if username and password and telephone:
        ...
        if res1:
            ...
            return jsonify(
                {"code": 2003, "msg": "输入的性别只能是 0(男) 或 1(女)！！！"}
            )
            ...
            sql3 = f"INSERT INTO user(username, password, role, sex, telephone, address) VALUES('{username}', '{password}', '1', '{sex}', '{telephone}', '{address}')"
            db.execute_db(sql3)
            print(f"新增用户信息 ==>> {sql3}")
            return jsonify({"code": 0, "msg": "恭喜，注册成功！"})
    else:
        return jsonify(
            {"code": 2001, "msg": "用户名/密码/手机号不能为空，请检查！！！"}
        )
...
def user_login():
    ...
    if username and password:  # 注意if条件中空串 "" 也是空, 按False处理
        ...
        sql2 = "SELECT * FROM user WHERE username = '{}' and password = '{}'".format(
        ...
        if res2:
            ...
            return jsonify(
                {"code": 0, "login_info": return_info, "msg": "恭喜，登录成功！"}
            )
...
def user_delete(id):
    ...
    if username and token:
        ...
        if redis_token:
            if (
                redis_token == token
            ):  # 如果从redis中取到的token不为空，且等于请求body中的token
                ...
                if user_role == 0:  # 如果当前登录用户是管理员用户
                    ...
                    if not res2:  # 如果要删除的用户不存在于数据库中，res2为空
                        return jsonify(
                            {
                                "code": 3005,
                                "msg": "删除的用户ID不存在，无法进行删除，请检查！！！",
                            }
                        )
                    elif (
                        res2[0]["role"] == 0
                    ):  # 如果要删除的用户是管理员用户，则不允许删除
                        return jsonify(
                            {
                                "code": 3006,
                                "msg": f"用户ID：【 {id} 】，该用户不允许删除！！！",
                            }
                        )
                    else:
        ...
        return jsonify(
            {"code": 3001, "msg": "管理员用户/token口令不能为空，请检查！！！"}
        )
